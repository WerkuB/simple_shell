...
int cant_open(char *file_path)
{
	...
	if (!hist_str)
		return (127);
	len = _strlen(shell) + _strlen(hist_str) + _strlen(file_path) + 16;
...
int proc_file_commands(char *file_path, int *execute_return)
{
	...
	if (file == -1)
	{
		*execute_return = cant_open(file_path);
	...
	variable_replacement(&line, execute_return);
	...
	for (i = 0; args[i]; i++)
	{
		if (_strncmp(args[i], ";", 1) == 0)
		{
			free(args[i]);
			args[i] = NULL;
			return_code = call_args(args, front, execute_return);
			args = &args[++i];
			i = 0;
		}
	}
